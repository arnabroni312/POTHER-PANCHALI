...
<body class="home">
    <div class="site-wrapper animsition" data-animsition-in="fade-in" data-animsition-out="fade-out">
        ...
        <!-- banner part starts -->
        <section class="hero bg-image" data-image-src="images/collage.jpg">
            <div class="hero-inner">
                <?php if (strlen($_SESSION['fosuid']>0)) {
   $admid=$_SESSION['fosuid'];
$ret=mysqli_query($con,"select * from tbluser where ID='$admid'");
$row=mysqli_fetch_array($ret);
$fname=$row['FirstName'];
$lname=$row['LastName'];

?>
<strong><p style="color: orange; text-align: center;  font-size: 30px">WELCOME <?php echo $fname;?> <?php echo $lname;?></p></strong>

<?php } ?>
                <div class="container text-center hero-text font-white">
                    <h1> <img src="images/logo.png" width="300" height="180"></h1>
                    <strong><h5   style="color: white;"class="font-white space-xs">Find your favourite delicious hot food!</h5></strong>
                     <form class="form-inline" method="post" name="se" action="category.php">

<button onclick="location.href='category.php'" type="submit" name="se" class="btn theme-btn btn-lg">View Full Menu</button>
</form>
        ...
        <section class="popular">
            <div class="container">
                ...
                <?php
                    $ret=mysqli_query($con,"select * from tblfood where spl='1'");
                    $cnt=1;
                ...
                ?>
                    <div class="col-xs-12 col-sm-6 col-md-4 food-item">
                        <div class="food-item-wrap">
                            <div class="figure-wrap bg-image"><img src="admin/itemimages/<?php echo $row['Image'];?>" width="400" height="180"></div>
                            <div class="content">
                                ...
                                <?php } else {?>
                                    ...
                                    <?php if(isset($_SESSION['cart_details']) && $_SESSION['cart_details'][$row['ID']]>0){?>
                                        <button type="submit" name="submit" class="btn theme-btn-dash pull-right add-item hidden" data-row-id="<?php echo $row['ID'];?>" id="add-item-<?php echo $row['ID'];?>">Order Now</button>
                                        <div class="order-control" id="order-control-<?php echo $row['ID'];?>">
                                            <button type="submit" name="submit" class="btn theme-btn-dash pull-right modify-qty" data-row-id="<?php echo $row['ID'];?>" id="item-plus-<?php echo $row['ID'];?>"><i class='fa fa-plus'></i></button>
                                            <span class="pull-right" style="color:orange;font-weight:bold;padding:5px;" id="item-qty-<?php echo $row['ID'];?>"><?php echo $_SESSION['cart_details'][$row['ID']]; ?></span>
                                            <button type="submit" name="submit" class="btn theme-btn-dash pull-right modify-qty" data-row-id="<?php echo $row['ID'];?>" id="item-minus-<?php echo $row['ID'];?>"><i class='fa fa-minus'></i></button>
                                        </div>
                                    <?php } else{?>
                                        <button type="submit" name="submit" class="btn theme-btn-dash pull-right add-item" data-row-id="<?php echo $row['ID'];?>" id="add-item-<?php echo $row['ID'];?>">Order Now</button>
